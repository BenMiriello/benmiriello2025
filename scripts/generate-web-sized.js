...
function printUsage() {
  console.log(`
Usage: node generate-web-sized.js [options]

Options:
  --album <name>    Process specific album (e.g., --album alps)
  --overwrite       Overwrite existing large images (default: skip existing)
  --help            Show this help message

Examples:
  node generate-web-sized.js                     # Process all albums
  node generate-web-sized.js --album rome        # Process only 'rome' album
  node generate-web-sized.js --overwrite         # Regenerate all large images
  node generate-web-sized.js --album alps --overwrite
`);
}
...
async function processAlbum(album, overwrite) {
  ...
  if (overwrite) {
    ...
    if (existingFiles.length > 0) {
      console.log(`  Cleared ${existingFiles.length} existing large images`);
    }
  }
...
async function main() {
  ...
  console.log('🖼️  Web-Sized Image Generator (1920px)');
